  ...
  <v-card>
    ...
    <v-card-text>
      <v-form ref="formRef" v-model="form.valid" lazy-validation>
        ...
        <v-textarea required hint="Paste a valid Array JSON object" :rules="form.outputsRule"
                    v-model="form.outputs" label="Outputs"></v-textarea>
...
const form = reactive({
  ...
  outputs: '',
  outputsRule: [(v: string) => isValidEntryDefinition(v)],
});
...
async function validate() {
  ...
  if (valid) {
    ...
    if (props.onEdit) {
      await manage_trigger.updateTrigger(props.trigger._id, form.name, form.description, perm, form.resourceServer, form.outputs);
    } else {
      await manage_trigger.createTrigger(form.name, form.description, props.serviceId, perm, form.resourceServer, form.outputs);
    }
    onClose();
  }
}
